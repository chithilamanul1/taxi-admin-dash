        ...
        <div className="min-h-screen bg-slate-50 pt-32 pb-20 px-4">
            <div className="max-w-5xl mx-auto">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                    <div>
                        <h1 className="text-3xl font-black text-slate-900 uppercase tracking-tight">My Bookings</h1>
                        <p className="text-gray-500">Manage and track your journeys</p>
                    </div>
                    {session && (
                        <div className="flex items-center gap-3 bg-white px-4 py-2 rounded-full shadow-sm border border-slate-900/10">
                            <div className="w-8 h-8 bg-slate-100 rounded-full flex items-center justify-center text-slate-700 font-bold">
                                {session.user.name?.[0]}
                            </div>
                ...
                <div className="flex gap-2 mb-8 border-b border-gray-200">
                    <button
                        ...
                        className={`px-6 py-3 font-bold text-sm uppercase tracking-widest border-b-2 transition-all ${activeTab === 'upcoming' ? 'border-slate-600 text-slate-900' : 'border-transparent text-gray-400 hover:text-slate-900'}`}
                    >
                        Upcoming ({activeBookings.length})
                    </button>
                    <button
                        onClick={() => setActiveTab('history')}
                        className={`px-6 py-3 font-bold text-sm uppercase tracking-widest border-b-2 transition-all ${activeTab === 'history' ? 'border-slate-600 text-slate-900' : 'border-transparent text-gray-400 hover:text-slate-900'}`}
                    >
                ...
                {loading ? (
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        {displayedBookings.map(booking => (
                            <Link key={booking._id} href={`/booking/${booking._id}`} className="block group">
                                <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-900/5 group-hover:shadow-md group-hover:border-slate-600/30 transition-all cursor-pointer h-full flex flex-col justify-between">
                    ...
                    <div className="text-center py-20 bg-white rounded-3xl border border-dashed border-gray-200">
                        ...
                        <Link href="/" className="inline-block bg-slate-900 text-white px-8 py-3 rounded-xl font-bold text-sm hover:bg-slate-800 transition-colors">
                            Book a Ride
                        </Link>
                    </div>
                )}
