    ...
    return (
        <div className="min-h-screen bg-white pb-20 transition-colors">
            ...
            <div className="container mx-auto px-6 -mt-16 relative z-10 max-w-7xl">
                {posts.length === 0 ? (
                    <div className="bg-white p-24 rounded-none border-8 border-black shadow-[20px_20px_0px_0px_#FACC15] text-center">
                        <p className="text-black text-2xl font-black uppercase italic tracking-widest leading-none">NO ARCHIVES FOUND</p>
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-12">
                        {posts.map((post) => (
                            <Link href={`/blog/${post.slug}`} key={post._id} className="group">
                                <article className="bg-white rounded-none border-4 border-black flex flex-col h-[600px] hover:border-black shadow-[12px_12px_0px_0px_#FACC15] hover:translate-y-[-12px] hover:shadow-[24px_24px_0px_0px_#FACC15] transition-all duration-500 overflow-hidden">
                                    <div className="relative aspect-video overflow-hidden border-b-4 border-black">
                                        <img
                                            src={post.imageUrl || '/logo.png'}
                                            alt={post.title}
                                            className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 scale-105 group-hover:scale-115 opacity-90 group-hover:opacity-100"
                                        />
                                    ...
                                    <div className="p-8 flex-1 flex flex-col pb-8">
                                        <div className="flex items-center gap-3 text-[10px] text-emerald-700 font-black mb-6 uppercase tracking-[0.4em] italic leading-none">
                                            <div className="w-2 h-2 bg-emerald-700 animate-pulse"></div>
                                            LOG ENTRY
                                        </div>
                                        <h2 className="text-3xl font-black text-black mb-6 group-hover:text-emerald-700 transition-colors line-clamp-2 uppercase italic tracking-tighter leading-none">
                                            {post.title}
                                        </h2>
                                        <p className="text-slate-600 text-sm font-bold line-clamp-3 mb-8 flex-1 uppercase tracking-widest leading-relaxed">
                                        ...
                                        <div className="flex items-center justify-between border-t-4 border-black pt-8 bg-slate-50 -mx-8 -mb-8 px-8 pb-8 transition-colors group-hover:bg-[#FACC15]/10">
                                            <span className="text-black font-black text-[10px] uppercase tracking-widest italic flex items-center gap-2">
                                                READ ARCHIVE
                                            </span>
