...
export default function SupportPage() {
    ...
    if (loading) return <div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin text-slate-950" size={40} /></div>
        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-24 pb-12 px-4 transition-colors">
            <div className="max-w-4xl mx-auto">
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-950 dark:text-white">Support Center</h1>
                        <p className="text-gray-500 dark:text-slate-400">How can we help you today?</p>
                    </div>
                    {view === 'list' && (
                        <button
                            onClick={() => setView('new')}
                            className="bg-amber-600 text-slate-950 px-4 py-2 rounded-lg font-bold flex items-center gap-2 hover:bg-yellow-400 transition-colors"
                        >
                    ...
                    {view !== 'list' && (
                        <button
                            ...
                            className="text-gray-500 hover:text-slate-950 font-medium"
                        >
                            ← Back to Tickets
                        </button>
                    )}
                ...
                {view === 'list' && (
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm overflow-hidden border border-slate-950/10 dark:border-white/5">
                        {tickets.length === 0 ? (
                                    ...
                                    >
                                        <div>
                                            <div className="flex items-center gap-3 mb-1">
                                                ...
                                                <h3 className="font-bold text-slate-950 dark:text-white">{ticket.subject}</h3>
                ...
                {view === 'new' && (
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm p-8 max-w-2xl mx-auto border border-slate-950/10 dark:border-white/5">
                        <h2 className="text-xl font-bold text-slate-950 dark:text-white mb-6">Create New Ticket</h2>
                        <form onSubmit={handleCreateTicket} className="space-y-4">
                            <div>
                                ...
                                <input
                                    required
                                    type="text"
                                    className="w-full px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-lg outline-none focus:ring-2 focus:ring-amber-600/20 text-slate-950 dark:text-white"
                            ...
                            <div>
                                ...
                                <textarea
                                    required
                                    className="w-full px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-lg outline-none focus:ring-2 focus:ring-amber-600/20 text-slate-950 dark:text-white h-32"
                            ...
                            <button
                                type="submit"
                                disabled={submitting}
                                className="w-full bg-slate-950 dark:bg-amber-600 text-white py-3 rounded-lg font-bold hover:bg-slate-950/90 dark:hover:bg-amber-500 transition-colors disabled:opacity-50"
                            >
                ...
                {view === 'detail' && selectedTicket && (
                    <div className="max-w-3xl mx-auto">
                        <div className="bg-white dark:bg-slate-900 rounded-t-2xl shadow-sm p-6 border-b border-slate-100 dark:border-white/5 flex justify-between items-start">
                            <div>
                                <h2 className="text-xl font-bold text-slate-950 dark:text-white mb-1">{selectedTicket.subject}</h2>
                            ...
                            {selectedTicket.messages.map((msg, idx) => (
                                <div key={idx} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-[80%] rounded-2xl p-4 ${msg.sender === 'user'
                                        ? 'bg-slate-950 dark:bg-amber-600 text-white rounded-tr-none'
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-b-2xl shadow-sm p-4 border-t border-slate-100 dark:border-white/5">
                            <div className="flex gap-2">
                                <input
                                    ...
                                    className="flex-1 px-4 py-3 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-xl outline-none focus:ring-2 focus:ring-amber-600/20 text-slate-950 dark:text-white"
                                    value={replyMessage}
                                ...
                                <button
                                    ...
                                    className="bg-amber-600 text-slate-950 p-3 rounded-xl hover:bg-yellow-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                >
