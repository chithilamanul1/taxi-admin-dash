                ...
                <div className="max-w-4xl mx-auto flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className={`w-14 h-14 ${isOnline ? 'bg-green-500' : 'bg-white/20'} rounded-full flex items-center justify-center text-2xl font-bold relative`}>
                            ...
                            {isOnline && (
                                <span className="absolute -bottom-1 -right-1 w-5 h-5 bg-green-400 rounded-full border-2 border-slate-900 flex items-center justify-center">
                    ...
                    <div className="bg-white rounded-xl p-4 shadow-sm relative overflow-hidden">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <p className={`text-2xl font-mono font-bold ${(driver?.walletBalance || 0) < 5000 ? 'text-red-600' : 'text-slate-900'}`}>
                                    Rs {(driver?.walletBalance || 0).toLocaleString()}
                                </p>
                            </div>
                            <div className="w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center text-slate-700 font-bold">
                                💰
                            </div>
                        </div>
                        <button
                            onClick={() => setShowTopupModal(true)}
                            className="w-full mt-3 bg-slate-900 text-white py-2 rounded-lg text-sm font-bold hover:bg-slate-800 transition-colors"
                        >
                            + Top Up
                        </button>
                    </div>
                ...
                <div className="flex gap-2 mb-6">
                    {['pending', 'ongoing', 'completed'].map(tab => (
                        <button
                            ...
                            className={`flex-1 py-3 rounded-xl font-bold text-sm uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-slate-900 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}
                        >
                        ...
                        filteredBookings.map(booking => (
                            <div key={booking._id} className="bg-white rounded-xl p-5 shadow-sm">
                                <div className="flex items-start justify-between mb-4">
                                    <div>
                                        <p className="font-bold text-slate-900">{booking.customerName || 'Guest'}</p>
                                ...
                                {booking.guestPhone && (
                                    <div className="flex gap-2 mb-4">
                                        <a
                                            ...
                                            className="flex-1 flex items-center justify-center gap-2 bg-slate-50 text-slate-700 py-3 rounded-xl font-bold text-sm border border-slate-100 hover:bg-slate-100 transition-colors"
                                        >
                                            <Phone size={16} />
                                            Call Guest
                                        </a>
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            {booking.flightNumber && (
                                                <div className="flex items-center gap-2 text-xs text-gray-600">
                                                    <span className="text-slate-600 text-[10px] font-black">✈️</span>
                                                    <span>Flight: <strong className="text-slate-900 font-black">{booking.flightNumber}</strong></span>
                                                </div>
                                            )}
                                    ...
                                    <div className="space-y-3 bg-slate-50 p-3 rounded-xl border border-slate-100">
                                        ...
                                        <div className="space-y-1.5">
                                            <div className="flex justify-between items-center text-xs">
                                                ...
                                                <span className="font-black text-slate-900">Rs {booking.totalPrice?.toLocaleString()}</span>
                                            </div>
                                    ...
                                    <div className="mt-6 space-y-4">
                                        ...
                                        <div className="rounded-2xl overflow-hidden border border-gray-200 shadow-inner h-48 relative bg-slate-50">
                                            {isLoaded && selectedBookingForMap?._id === booking._id ? (
                                                ...
                                                <div className="w-full h-full flex flex-col items-center justify-center gap-3">
                                                    <div className="w-12 h-12 bg-white rounded-full shadow-md flex items-center justify-center text-slate-600">
                                                        <MapPin size={24} />
                                                    </div>
                                                    <button
                                                        onClick={() => {
                                        ...
                                        {booking.status === 'ongoing' && (
                                            <button
                                                ...
                                                className="w-full bg-slate-600 hover:bg-slate-700 text-white py-4 rounded-2xl font-black flex items-center justify-center gap-3 transition-all active:scale-[0.98] shadow-lg shadow-amber-500/20"
                                            >
                                                <CheckCircle size={20} />
                                                COMPLETE TRIP
                                            </button>
                        ...
                        <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-4">
                                <button
                                    ...
                                    className={`py-3 rounded-xl font-bold border-2 transition-all ${topupAmount === '5000' ? 'border-amber-500 bg-slate-50 text-slate-700' : 'border-gray-200 text-gray-500 hover:border-amber-200'}`}
                                >
                                    Rs 5,000
                                </button>
                                <button
                                    onClick={() => { setTopupAmount('10000'); setCustomAmount(''); }}
                                    className={`py-3 rounded-xl font-bold border-2 transition-all ${topupAmount === '10000' ? 'border-amber-500 bg-slate-50 text-slate-700' : 'border-gray-200 text-gray-500 hover:border-amber-200'}`}
                                >
                                    Rs 10,000
                                </button>
                            </div>
                            ...
                            <div className="mb-6">
                                ...
                                >
                                    ...
                                    <span className={topupAmount === 'custom' ? 'text-slate-900 font-bold' : 'text-gray-500'}>Custom Amount</span>
                                </button>
                                ...
                                {topupAmount === 'custom' && (
                                    <div className="animate-fade-in-down">
                                        <input
                                            ...
                                            className="w-full p-3 border-2 border-slate-100 rounded-xl font-mono text-lg font-bold outline-none focus:border-amber-500"
                                            value={customAmount}
                            ...
                            <button
                                ...
                                className="w-full py-4 bg-slate-900 text-white rounded-xl font-bold text-lg hover:bg-slate-800 transition-colors flex justify-center items-center gap-2 shadow-lg shadow-slate-900/20"
                            >
