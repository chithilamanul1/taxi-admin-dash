...
export async function PUT(request) {
    try {
        ...
        const operations = packages.map(pkg => {
            ...
            return {
                updateOne: {
                    ...
                    update: { 
                        $set: updateData,
                        $setOnInsert: { id: id || `pkg-apt-${hours}h-${pkg.vehicleType}-${Date.now()}-${Math.random().toString(36).substr(2, 5)}` }
                    },
                    upsert: true
                }
            };
        ...
        return NextResponse.json({ success: true, message: `Airport Group ${hours}H isolated save complete.` });
    } catch (error) {
...
export async function DELETE(request) {
    try {
        ...
        await AirportRoundTour.deleteMany({ hours: Number(hours) });
        return NextResponse.json({ success: true, message: `Airport Group ${hours}H deleted.` });
    } catch (error) {
        return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    }
}
