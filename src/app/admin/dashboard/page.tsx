...
export default function AdminDashboard() {
    const stats = [
        ...
        { label: "Active Drivers", value: "32", icon: <Users className="text-amber-600" />, trend: "Steady" },
    ...
    return (
        <div className="min-h-screen bg-slate-50 text-slate-950 pt-24 pb-12 px-8">
                ...
                <div className="flex flex-col md:flex-row justify-between items-end gap-6">
                    <div className="space-y-2">
                        <h1 className="text-4xl font-black tracking-tight flex items-center gap-3">
                            Dispatcher <span className="text-amber-600 underline underline-offset-8 decoration-amber-600/30">Control Centre</span>
                        </h1>
                    ...
                    <div className="flex gap-3">
                        ...
                        <button className="px-5 py-2.5 bg-amber-600 text-black rounded-xl text-sm font-bold hover:scale-105 transition-all shadow-lg shadow-amber-600/20">
                            New Direct Booking
                        </button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    {stats.map((stat, idx) => (
                        <Card key={idx} className="relative overflow-hidden group bg-white border-slate-200 shadow-sm transition-all hover:shadow-md">
                            <div className="absolute top-0 right-0 w-24 h-24 bg-amber-600/5 blur-3xl -mr-12 -mt-12 group-hover:bg-amber-600/10 transition-colors" />
                    ...
                    <CardHeader className="flex md:flex-row justify-between items-center border-b border-slate-100 p-8">
                        ...
                        <div className="flex gap-4 mt-4 md:mt-0">
                            <div className="relative group">
                                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-amber-600 transition-colors" size={18} />
                                <input
                                    placeholder="Search ref, customer, or location..."
                                    className="bg-white border border-slate-200 rounded-xl py-2.5 pl-10 pr-4 text-sm focus:outline-none focus:border-amber-600 transition-all w-64 shadow-sm"
                                />
                            </div>
                            <button className="p-2.5 bg-white border border-slate-200 rounded-xl text-slate-500 hover:text-slate-950 transition-colors shadow-sm">
                                <Filter size={18} />
                            </button>
                        </div>
                    </CardHeader>
                    <CardContent className="p-0">
                        <div className="overflow-x-auto">
                            <table className="w-full">
                                <thead>
                                    <tr className="bg-slate-50 border-b border-slate-200 text-slate-950">
                                    ...
                                    {ACTIVE_BOOKINGS.map((booking) => (
                                        <tr key={booking.id} className="hover:bg-slate-50/50 transition-colors group border-b border-slate-100">
                                            <td className="px-8 py-6">
                                                <span className="font-mono text-xs text-amber-600 bg-amber-600/10 px-2 py-1 rounded border border-amber-600/10">#{booking.id.toUpperCase()}</span>
                                            </td>
                                            <td className="px-8 py-6">
                                            ...
                                            <td className="px-8 py-6 text-center">
                                                <button className="p-2 hover:bg-slate-100 rounded-lg transition-colors text-slate-400 hover:text-slate-950">
