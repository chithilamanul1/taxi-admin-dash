...
export default function TripPlannerClient({ isEmbedded = false }: { isEmbedded?: boolean }) {
    ...
    const handleDownloadPDF = () => {
        ...
        // Header
        doc.setFontSize(22);
        doc.setTextColor(5, 46, 22); // emerald-950
        doc.text(itinerary.title, 15, 25);

        doc.setFontSize(10);
        doc.setTextColor(100);
        doc.text(`Duration: ${formData.duration} Days | Travelers: ${formData.travelers}`, 15, 32);

        // Description
        doc.setFontSize(12);
        doc.setTextColor(80);
        const splitDescription = doc.splitTextToSize(itinerary.description, pageWidth - 30);
        doc.text(splitDescription, 15, 45);

        // Destinations
        doc.setFontSize(14);
        doc.setTextColor(5, 46, 22);
        doc.text('Planned Destinations', 15, 65);
        
        let destY = 72;
        itinerary.destinations?.forEach((dest: string, i: number) => {
            doc.setFontSize(11);
            doc.setTextColor(60);
            doc.text(`${i + 1}. ${dest}`, 20, destY);
            destY += 7;
        });

        // Itinerary Table
        const tableData = itinerary.days?.map((day: any) => [
            `Day ${day.day}`,
            day.location,
            day.title,
            day.activities?.join(', ') || ''
        ...
        doc.text('Generated by Airport Taxis Tours Sri Lanka', pageWidth / 2, finalY + 20, { align: 'center' });
        doc.text('www.srilankantaxi.lk', pageWidth / 2, finalY + 26, { align: 'center' });

        doc.save(`${itinerary.title.replace(/\s+/g, '_')}_Itinerary.pdf`);
    };
        ...
        return (
            <div className={`${isEmbedded ? 'bg-transparent pb-10 px-0' : 'min-h-screen bg-white pt-24 pb-20 px-6'}`}>
                                ...
                                <div className="mt-12 pt-12 border-t border-emerald-200">
                                    ...
                                    <div className="space-y-4">
                                        <button
                                            ...
                                            className="w-full py-5 bg-emerald-500 text-white rounded-2xl font-black transition-transform hover:scale-[1.02] shadow-xl flex items-center justify-center gap-2"
                                        >
                ...
                <TourBookingModal 
                    ...
                    tourTitle={`AI Custom Trip: ${itinerary.title}`}
                    tourId="ai-custom-trip"
                    duration={formData.duration}
                    price={65} // Fixed daily rate for custom trips
                    currency="USD"
                />
            </div>
        );
