        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-2xl shadow-2xl flex flex-col max-h-[90vh]">
                <div className="p-6 border-b dark:border-white/10 flex justify-between items-center">
                    <h3 className="text-xl font-bold text-slate-950 dark:text-white">Assign Driver</h3>
                    <button onClick={onClose} className="p-2 hover:bg-slate-100 dark:hover:bg-white/5 rounded-full"><X size={20} /></button>
                </div>

                <div className="p-6 flex-1 overflow-y-auto">
                    <div className="mb-4 bg-amber-50 dark:bg-slate-950/20 p-4 rounded-xl">
                        <h4 className="font-bold text-slate-950 dark:text-amber-400 text-sm mb-1">Booking #{booking._id.slice(-6)}</h4>
                                ...
                                return (
                                    <div
                                        ...
                                        className={`p-3 rounded-xl border transition-all flex justify-between items-center ${!hasFunds ? 'opacity-50 cursor-not-allowed bg-slate-50' : 'cursor-pointer'} ${selectedDriver?._id === driver._id
                                                ? 'border-amber-500 bg-amber-50 dark:bg-slate-950/30 ring-1 ring-amber-500'
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 text-xs text-slate-500">
                                                    ...
                                                    <span className={`font-mono font-bold ${hasFunds ? 'text-amber-600' : 'text-red-500'}`}>
                                        ...
                                        <div className="text-right">
                                            ...
                                            {selectedDriver?._id === driver._id && <Check size={16} className="text-amber-600 ml-auto mt-1" />}
                                        </div>
                                    </div>
                                );
                            })}
                ...
                <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900/50 border-t dark:border-white/10">
                    <div className="flex justify-between items-center mb-4 text-xs">
                        ...
                        <span className="text-amber-600">Commission (10%): <strong>- Rs {commissionAmount.toLocaleString()}</strong></span>
                    </div>
                    <div className="flex gap-3">
                        ...
                        <button
                            ...
                            className="flex-[2] py-3 bg-slate-950 text-white rounded-xl font-bold hover:bg-amber-800 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-colors"
                        >
