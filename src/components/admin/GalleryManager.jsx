...
import { Image as ImageIcon, Plus, Trash2, Edit2, Loader2, X, Check, Search, Tag } from 'lucide-react';
...
export default function GalleryManager() {
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            ...
            const data = await res.json();
            if (data.success) {
                setImages(images.map(img => img._id === editImage._id ? data.data : img));
                            ...
                            <div className="aspect-[4/5] relative">
                                ...
                                <div className="absolute top-4 right-4 flex gap-2 translate-y-[-10px] group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
                                    <button
                                        ...
                                        className="p-3 bg-blue-500 text-white rounded-xl shadow-lg hover:bg-blue-600 active:scale-90 transition-all"
                                    >
                                        <Edit2 size={18} />
                                    </button>
                                    <button
                ...
                {showEditModal && editImage && (
                    ...
                    >
                        ...
                        >
                            <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                                <div>
                                    <h3 className="text-2xl font-black text-slate-800 uppercase tracking-tighter">Edit Masterpiece</h3>
                                    <p className="text-xs text-slate-500 font-bold uppercase tracking-widest mt-1">Update text or category</p>
                                </div>
                                <button onClick={() => { setShowEditModal(false); setEditImage(null); }} className="p-3 hover:bg-slate-200 rounded-2xl transition-all text-slate-400">
                                    <X size={24} />
                                </button>
                            </div>
                            ...
                            <form onSubmit={handleUpdate} className="p-8 space-y-6">
                                <div>
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 drop-shadow-sm">Display Caption Text</label>
                                    <textarea
                                        value={editImage.caption}
                                        onChange={(e) => setEditImage({ ...editImage, caption: e.target.value })}
                                        className="w-full bg-slate-50 border-2 border-slate-200 px-6 py-4 rounded-2xl block text-slate-700 font-bold mt-2 resize-none h-24 focus:border-red-500 focus:bg-white transition-all outline-none"
