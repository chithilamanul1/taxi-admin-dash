...
const RentalsWidget = () => {
    return (
        <div className="animate-fade-in text-center py-8">
            <h3 className="text-2xl font-bold text-slate-900 mb-2">Rent a Vehicle</h3>
            <p className="text-slate-900/60 mb-8 max-w-lg mx-auto">Choose from our wide range of premium vehicles for your personal travel needs.</p>

            <div className="grid md:grid-cols-2 gap-6 max-w-2xl mx-auto">
                {RENTALS.map((item) => (
                    <div key={item.id} className="bg-slate-50/50 border border-slate-900/10 p-6 rounded-2xl hover:border-slate-600 hover:shadow-lg transition-all text-left">
                        <div className="w-12 h-12 bg-white rounded-xl shadow-sm flex items-center justify-center mb-4 text-slate-600">
                            <item.icon size={24} />
                        </div>
                        ...
                        <div className="space-y-2 mb-6">
                            {item.features.map((feat, i) => (
                                <div key={i} className="flex items-center gap-2 text-sm text-slate-900/80">
                                    <CheckCircle2 size={14} className="text-slate-600" /> {feat}
                                </div>
                            ))}
                        </div>
            ...
            <div className="mt-8 p-4 bg-slate-900/10 rounded-xl inline-flex items-center gap-4 text-slate-900 font-bold text-sm">
                <Phone size={16} />
