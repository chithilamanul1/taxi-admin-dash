...
const RoundTripBooking = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto bg-white rounded-[2.5rem] shadow-2xl overflow-hidden border border-slate-100">
      <div className="flex bg-slate-900 p-2 border-b border-slate-800 overflow-x-auto gap-2 [&::-webkit-scrollbar]:hidden" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        <button onClick={() => { setTab('airport-round-tour'); syncWithCalculator('airport-round-tour', selectedVehicle); }} className={`flex-1 flex items-center justify-center gap-2 py-4 px-3 whitespace-nowrap rounded-2xl transition-all font-black text-[10px] uppercase tracking-[0.1em] ${tab === 'airport-round-tour' ? 'bg-slate-800 text-white shadow-sm border border-slate-700' : 'text-slate-400 hover:text-slate-300 hover:bg-slate-800/30'}`}><Plane size={16} /> Airport Round Tour</button>
        <button onClick={() => { setTab('normal-round-tour'); syncWithCalculator('normal-round-tour', selectedVehicle); }} className={`flex-1 flex items-center justify-center gap-2 py-4 px-3 whitespace-nowrap rounded-2xl transition-all font-black text-[10px] uppercase tracking-[0.1em] ${tab === 'normal-round-tour' ? 'bg-slate-800 text-white shadow-sm border border-slate-700' : 'text-slate-400 hover:text-slate-300 hover:bg-slate-800/30'}`}><Sparkles size={16} /> Normal Round Tour</button>
        <button onClick={() => { setTab('destination-based-tour'); syncWithCalculator('destination-based-tour', selectedVehicle); }} className={`flex-1 flex items-center justify-center gap-2 py-4 px-3 whitespace-nowrap rounded-2xl transition-all font-black text-[10px] uppercase tracking-[0.1em] ${tab === 'destination-based-tour' ? 'bg-slate-800 text-white shadow-sm border border-slate-700' : 'text-slate-400 hover:text-slate-300 hover:bg-slate-800/30'}`}><MapPin size={16} /> Destination-Based Tour</button>
      </div>
