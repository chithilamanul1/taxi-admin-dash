                                ...
                                <div className="space-y-6">
                                    {tour.itinerary?.map((item, index) => (
                                        <div key={item.day} className={`relative pl-8 pb-8 border-l-2 ${index === (tour.itinerary?.length - 1 || 0) ? 'border-transparent' : 'border-emerald-100 dark:border-slate-800'}`}>
                                            <div className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full border-2 transition-all ${activeDay === item.day ? 'bg-emerald-600 border-emerald-600' : 'bg-white dark:bg-slate-800 border-emerald-200 dark:border-slate-600'}`} />
                                            <div className="flex flex-col gap-2">
                                                <button onClick={() => setActiveDay(activeDay === item.day ? null : item.day)} className="flex items-center gap-4 text-left group">
                        ...
                        {tour.mapImage && (
                            <div className="bg-white dark:bg-slate-900 rounded-3xl p-2 shadow-xl border border-emerald-900/5 dark:border-slate-800 overflow-hidden">
                                <div className="relative aspect-[4/3] rounded-2xl overflow-hidden bg-slate-100 dark:bg-slate-800">
                                    <Image src={tour.mapImage} alt="Tour Route Map" fill className="object-cover hover:scale-105 transition-transform duration-700" />
                                ...
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-3 bg-emerald-900/50 rounded-lg p-1 border border-emerald-700/50">
                                            <button onClick={() => setMemberCount(prev => ({ ...prev, adults: Math.max(1, prev.adults - 1) }))} className="w-8 h-8 flex items-center justify-center text-white hover:bg-emerald-700 rounded-md transition-colors"><Minus size={14} /></button>
                                            <span className="text-white font-bold w-4 text-center">{memberCount.adults}</span>
                                            <button onClick={() => setMemberCount(prev => ({ ...prev, adults: prev.adults + 1 }))} className="w-8 h-8 flex items-center justify-center text-white hover:bg-emerald-700 rounded-md transition-colors"><Plus size={14} /></button>
                                        </div>
                                    </div>
                                    <div className="flex items-center justify-between">
                                        <span className="text-emerald-100 font-medium text-sm">Children</span>
                                        <div className="flex items-center gap-3 bg-emerald-900/50 rounded-lg p-1 border border-emerald-700/50">
                                            <button onClick={() => setMemberCount(prev => ({ ...prev, children: Math.max(0, prev.children - 1) }))} className="w-8 h-8 flex items-center justify-center text-white hover:bg-emerald-700 rounded-md transition-colors"><Minus size={14} /></button>
                                            <span className="text-white font-bold w-4 text-center">{memberCount.children}</span>
                                            <button onClick={() => setMemberCount(prev => ({ ...prev, children: prev.children + 1 }))} className="w-8 h-8 flex items-center justify-center text-white hover:bg-emerald-700 rounded-md transition-colors"><Plus size={14} /></button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div className="space-y-4 relative z-10">
                                <button onClick={() => setIsModalOpen(true)} className="flex items-center justify-center gap-3 w-full py-4 bg-white hover:bg-slate-50 text-emerald-950 rounded-xl font-bold shadow-lg shadow-emerald-900/20 transition-all hover:scale-[1.02]"><Calendar size={20} /> Book Now</button>
                                <a href={`https://wa.me/+94716885880?text=${encodeURIComponent(`Hi, I'm interested in booking the "${tour.title}".\n\nPax: ${memberCount.adults} Adults, ${memberCount.children} Children\nPlease provide availability and a quote.`)}`} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-3 w-full py-4 bg-[#25D366] hover:bg-[#20bd5a] text-white rounded-xl font-bold shadow-lg shadow-emerald-900/20 transition-all hover:scale-[1.02]"><MessageCircle size={20} /> WhatsApp Inquiry</a>
                            </div>
                        </div>

                        <TourBookingModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} tourTitle={tour.title} tourId={tour.id} duration={tour.duration} price={Math.round(tour.price * 300)} currency="LKR" />
                            ...
                            <ul className="space-y-4">
                                {tour.includes?.map((item, i) => (
                                    <li key={i} className="flex gap-3 text-sm text-slate-600 dark:text-slate-400 group">
                                        <div className="shrink-0 w-5 h-5 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center group-hover:bg-emerald-200 dark:group-hover:bg-emerald-900/50 transition-colors"><Check size={12} className="text-emerald-600 dark:text-emerald-400" /></div>
