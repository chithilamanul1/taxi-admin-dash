...
export default function RecentPosts() {
    ...
    return (
        <section className="py-32 bg-white relative transition-colors duration-300 border-t-8 border-black overflow-hidden">
            {/* Background Accent */}
            ...
            <div className="container mx-auto px-6 max-w-7xl relative z-10">
                <div className="flex flex-col md:flex-row items-end justify-between mb-20 gap-8 animate-slide-up">
                    <div className="max-w-2xl bg-black p-8 md:p-12 border-4 border-black shadow-[12px_12px_0px_0px_#FACC15]">
                        ...
                        <p className="text-[#FACC15] font-black uppercase tracking-[0.3em] text-[10px] md:text-xs italic leading-relaxed">Discover Sri Lanka through the eyes of our expert travel guides and professional chauffeurs.</p>
                    </div>
                    ...
                    <div className="grid md:grid-cols-3 gap-12">
                        {posts.map((post, idx) => (
                            <Link
                                href={`/blog/${post.slug}`}
                                key={post._id}
                                className="group relative bg-white rounded-none overflow-hidden border-4 border-black flex flex-col h-[600px] animate-slide-up hover:border-[#FACC15] shadow-[12px_12px_0px_0px_#facc15] hover:translate-y-[-10px] hover:shadow-[20px_20px_0px_0px_#facc15] transition-all duration-500"
                            ...
                            >
                                ...
                                <div className="p-8 flex flex-col flex-1 pb-4">
                                    <h3 className="text-2xl font-black mb-4 leading-tight text-black group-hover:text-[#FACC15] transition-colors line-clamp-2 uppercase tracking-tighter italic">
                                        {post.title}
                                    </h3>
                                    <p className="text-black/60 text-[11px] font-bold leading-relaxed line-clamp-3 mb-6 uppercase tracking-wider">
                                        {post.seo?.metaDescription || post.excerpt || "Read more about this fascinating insight into Sri Lankan travel."}
                                    </p>
                                </div>

                                <div className="flex items-center justify-between border-t-4 border-black px-8 py-8 mt-auto bg-slate-50 group-hover:bg-[#FACC15]/10 transition-colors">
