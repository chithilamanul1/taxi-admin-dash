...
export default function RecentPosts() {
    ...
    return (
        <section className="py-32 bg-white dark:bg-emerald-900 relative transition-colors">
                    ...
                    <div className="grid md:grid-cols-3 gap-8">
                        {posts.map((post, idx) => (
                            <Link
                                href={`/blog/${post.slug}`}
                                key={post._id}
                                className="group relative bg-white dark:bg-emerald-900 rounded-3xl overflow-hidden border border-emerald-900/10 dark:border-white/5 flex flex-col h-[520px] animate-slide-up hover:border-emerald-600 shadow-sm hover:shadow-xl transition-all duration-500"
