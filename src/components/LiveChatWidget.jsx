        ...
        <div className="fixed bottom-6 right-6 md:bottom-8 md:right-8 w-[90vw] md:w-[380px] h-[70vh] md:h-[500px] bg-white dark:bg-slate-900 rounded-[2rem] shadow-2xl border border-slate-200 dark:border-slate-800 z-[70] flex flex-col overflow-hidden animate-fade-in-up">
            ...
            {status === 'authenticated' ? (
                    ...
                    <div className="flex-1 overflow-y-auto p-5 space-y-4 custom-scrollbar bg-slate-50 dark:bg-slate-950/50">
                        {messages.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-center opacity-50">
                                <MessageCircle size={40} className="text-slate-900/20 mb-3" />
                                <p className="text-xs font-bold text-slate-900/40 uppercase tracking-widest">How can we help you today?</p>
                            </div>
                        ) : (
                            messages.map((msg) => (
                                <div
                    ...
                    <form onSubmit={sendMessage} className="p-4 pb-8 md:pb-6 bg-white dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 shrink-0">
                        <div className="relative flex items-center gap-2">
                            ...
                            <button
                                ...
                                className="absolute right-1 w-10 h-10 bg-slate-600 text-white rounded-lg flex items-center justify-center hover:bg-slate-700 transition-colors disabled:opacity-50"
                            >
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center bg-slate-50 dark:bg-slate-900">
                    <div className="w-16 h-16 bg-slate-100 dark:bg-slate-900/20 rounded-full flex items-center justify-center mb-6 animate-pulse">
                        <Lock className="text-slate-600 dark:text-amber-400" size={32} />
                    </div>
                    ...
                    <button
                        onClick={() => signIn()}
                        className="w-full py-3 bg-slate-600 text-white rounded-xl font-bold shadow-lg shadow-slate-600/20 hover:bg-slate-700 hover:scale-[1.02] transition-all flex items-center justify-center gap-2"
                    >
                        <User size={18} />
                        Sign In / Register
                    </button>
