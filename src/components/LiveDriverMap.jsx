    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-slate-950 dark:text-white">Live Driver Tracking</h2>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 rounded-xl font-medium transition-all ${autoRefresh
                            ? 'bg-amber-100 text-amber-700 dark:bg-slate-950/30 dark:text-amber-400'
            ...
            {/* Google Map View */}
            <div className="w-full h-[400px] bg-slate-100 rounded-3xl overflow-hidden border border-slate-950/10 relative">
                {isLoaded ? (
                    <GoogleMap
                        ...
                        {drivers.map(driver => (
                            driver.currentLocation && driver.currentLocation.lat && (
                                <Marker
                                    ...
                                    title={`${driver.name} (${driver.isOnline ? 'Online' : 'Offline'})`}
                                    icon={{
                                        path: "M18.92 6.01C18.72 5.42 18.16 5 17.5 5h-11c-.66 0-1.21.42-1.42 1.01L3 12v8c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-1h12v1c0 .55.45 1 1 1h1c.55 0 1-.45 1-1v-8l-2.08-5.99zM6.5 16c-.83 0-1.5-.67-1.5-1.5S5.67 13 6.5 13s1.5.67 1.5 1.5S7.33 16 6.5 16zm11 0c-.83 0-1.5-.67-1.5-1.5s.67-1.5 1.5-1.5 1.5.67 1.5 1.5-.67 1.5-1.5 1.5zM5 11l1.5-4.5h11L19 11H5z",
                                        fillColor: driver.isOnline ? "#10b981" : "#64748b", // Emerald for Online, Slate for Offline
                                        fillOpacity: 1,
                                        strokeWeight: 1,
                                        strokeColor: "#ffffff",
                                        scale: 1.5,
            ...
            <div className="grid grid-cols-3 gap-4">
                <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-slate-800">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-amber-100 dark:bg-slate-950/30 rounded-xl flex items-center justify-center">
                            <User size={20} className="text-amber-600" />
                        </div>
                        <div>
                            <p className="text-2xl font-bold text-slate-950 dark:text-white">{drivers.length}</p>
            ...
            {onlineDrivers.map(driver => (
                <div key={driver._id} className="bg-white p-4 border rounded-xl mb-2 flex justify-between shadow-sm">
                    ...
                    {driver.currentLocation && (
                        <button onClick={() => openInGoogleMaps(driver.currentLocation.lat, driver.currentLocation.lng)} className="text-xs bg-amber-100 text-amber-700 px-3 py-1 rounded-lg">
                            View Link
                        </button>
                    )}
                </div>
