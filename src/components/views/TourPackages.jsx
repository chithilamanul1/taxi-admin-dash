    ...
    return (
        <div className="pb-20">
                        ...
                        {tours.map((pkg) => (
                            <div key={pkg._id || pkg.id} itemScope itemType="https://schema.org/Product" className="bg-white rounded-2xl shadow-xl overflow-hidden group hover:shadow-2xl transition-all duration-300 border border-slate-100 flex flex-col h-full">
                                ...
                                <Link href={`/tour-packages/${pkg.slug || pkg.id}`} className="block h-56 overflow-hidden relative">
                                    ...
                                    />
                                    <div className="absolute top-3 right-3 bg-slate-900/90 text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider backdrop-blur-sm shadow flex items-center gap-1">
                                        <Clock size={10} /> {typeof pkg.duration === 'object' ? `${pkg.duration.days}D / ${pkg.duration.nights}N` : pkg.duration}
                                    </div>
                                    {pkg.isFeatured && (
                                        <div className="absolute top-3 left-3 bg-yellow-400 text-slate-900 text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider shadow">
                                            Featured
                                        </div>
                                    )}
                                </Link>
                                ...
                                <div className="p-5 flex flex-col flex-grow">
                                    <Link href={`/tour-packages/${pkg.slug || pkg.id}`} className="group-hover:text-slate-700 transition-colors">
                                        <h3 itemProp="name" className="text-lg font-bold text-slate-900 mb-2 line-clamp-2 leading-tight h-12">{pkg.title}</h3>
                                    </Link>
                                    ...
                                    <div className="mt-auto pt-4 border-t border-gray-100 flex items-end justify-between">
                                        ...
                                        <div className="flex gap-2">
                                            <Link
                                                ...
                                                className="bg-slate-100 text-slate-900 p-2 rounded-lg hover:bg-slate-200 transition-colors"
                                                title="View Details"
                                            >
                                                <ArrowRight size={18} />
                                            </Link>
            ...
            <div className="text-center mt-16 max-w-2xl mx-auto px-4">
                <h3 className="text-2xl font-bold text-slate-900 mb-3">Looking for something else?</h3>
                ...
                <Link href="/contact" className="inline-block bg-slate-600 text-white font-bold px-8 py-3 rounded-xl hover:bg-slate-700 shadow-lg shadow-slate-600/20 transition-all hover:scale-105">
                    Plan My Custom Tour
                </Link>
            </div>
        </div>
