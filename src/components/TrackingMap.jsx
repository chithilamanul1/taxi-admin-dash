...
const TrackingMap = ({ pickup, dropoff, driverId }) => {
    ...
    return (
        <div className="space-y-4">
            <div className="w-full h-[400px] rounded-2xl overflow-hidden border border-slate-950/10 shadow-lg relative">
                <GoogleMap
                ...
                {!driverId && (
                    <div className="absolute bottom-4 left-4 right-4 bg-white/90 backdrop-blur-md p-3 rounded-xl border border-slate-950/10 text-sm text-center font-bold text-slate-500">
            ...
            {driverData && (
                <div className="bg-amber-50 border border-amber-100 p-4 rounded-xl flex items-center justify-between animate-fade-in">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center text-amber-600 shadow-sm font-bold text-lg">
                            {driverData.name?.charAt(0)}
                        </div>
                        <div>
                            <p className="text-xs text-slate-950/50 font-bold uppercase tracking-wider">Your Driver</p>
                            <p className="text-slate-950 font-bold text-lg">{driverData.name}</p>
                            <p className="text-sm text-amber-700">{driverData.vehicleNumber}</p>
                        </div>
                    </div>
