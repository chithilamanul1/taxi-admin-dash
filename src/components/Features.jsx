            ...
            <div className="container mx-auto px-6 relative z-10">
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {features.map((feature, idx) => (
                        <div
                            key={idx}
                            className={`stat-card group animate-slide-up p-4 rounded-xl border border-black dark:border-white/20 shadow-sm hover:shadow-md transition-shadow bg-white dark:bg-zinc-900`}
                            style={{ animationDelay: `${idx * 0.1}s` }}
                        >
                            <div className={`w-10 h-10 flex items-center justify-center mb-3 transition-all rounded-lg bg-[#FACC15] text-black group-hover:bg-black group-hover:text-[#FACC15]`}>
                                {React.cloneElement(feature.icon, { className: "w-5 h-5" })}
                            </div>
                            <h3 className={`text-lg md:text-xl font-black mb-2 transition-colors uppercase tracking-tight leading-none text-black dark:text-white group-hover:text-emerald-700 dark:group-hover:text-emerald-400`}>
                                {feature.title}
                            </h3>
                            <p className={`text-[9px] font-bold uppercase tracking-widest leading-relaxed transition-colors text-black/60 dark:text-white/60 group-hover:text-black/80 dark:group-hover:text-white/80`}>
                                {feature.description}
                            </p>
                        </div>
                    ))}
