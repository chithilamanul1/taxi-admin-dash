    ...
    return (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-0 sm:p-4 md:p-8 overflow-hidden touch-none no-scrollbar">
            ...
            <div className="relative w-full h-full sm:h-auto sm:max-h-[95vh] max-w-4xl bg-white sm:rounded-[2rem] md:rounded-[3rem] shadow-2xl overflow-hidden animate-zoom-in flex flex-col">
                {/* Header */}
                <div className="bg-slate-900 p-6 md:p-12 text-white relative flex-shrink-0 pt-10 sm:pt-6">
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-12">
                    <form onSubmit={handleSubmit} className="space-y-6 md:space-y-10">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-10">
                            {[
                                ...
                                <div key={field.key} className="space-y-2.5">
                                    <label className="text-[10px] md:text-xs font-bold text-slate-900/40 uppercase tracking-widest ml-1">{field.label}</label>
                                    <input
                                        required={field.required}
                                        type={field.type}
                                        value={formData[field.key]}
                                        onChange={(e) => setFormData({ ...formData, [field.key]: e.target.value })}
                                        className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-200 rounded-2xl focus:outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all font-bold text-slate-900 placeholder:text-slate-300"
                        ...
                        <div className="space-y-2.5">
                            <label className="text-[10px] md:text-xs font-bold text-slate-900/40 uppercase tracking-widest ml-1">Billing Address</label>
                            <textarea
                                required
                                rows="2"
                                value={formData.billingAddress}
                                onChange={(e) => setFormData({ ...formData, billingAddress: e.target.value })}
                                className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-200 rounded-2xl focus:outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all resize-none font-bold text-slate-900 placeholder:text-slate-300"
                        ...
                        <div className="pt-2 md:pt-6">
                            <button
                                disabled={loading}
                                type="submit"
                                className="w-full py-6 md:py-7 bg-slate-900 text-white rounded-[2rem] md:rounded-[2.5rem] font-black text-xl md:text-2xl hover:bg-slate-800 transition-all shadow-2xl shadow-slate-900/20 flex items-center justify-center gap-4 group disabled:opacity-70"
                            >
                                {loading ? (
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-4 pt-4 text-slate-900/40">
