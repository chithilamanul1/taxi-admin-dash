    ...
    return (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-0 sm:p-4 md:p-8 overflow-hidden touch-none no-scrollbar">
            ...
            <div className="relative w-full h-full sm:h-auto sm:max-h-[95vh] max-w-4xl bg-white sm:rounded-[2rem] md:rounded-[3rem] shadow-2xl overflow-hidden animate-zoom-in flex flex-col">
                {/* Header */}
                <div className="bg-slate-950 p-6 md:p-12 text-white relative flex-shrink-0 pt-10 sm:pt-6">
                    ...
                    <div className="flex items-center gap-2 text-amber-400 text-[9px] md:text-[10px] font-bold uppercase tracking-widest mb-2 md:mb-5">
                    ...
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 md:gap-6">
                        <div>
                            ...
                            <p className="text-[10px] md:text-lg text-amber-200/80 font-medium">{product?.title}</p>
                        </div>
                        ...
                        <div className="flex flex-col items-end gap-2 bg-white/5 w-fit px-4 md:px-6 py-2 md:py-3 rounded-xl md:rounded-2xl border border-white/10 backdrop-blur-md">
                            <div className="flex items-baseline gap-2">
                                ...
                                <span className="text-amber-400 font-bold uppercase text-[8px] md:text-[11px] tracking-widest">
                                ...
                                <div className="flex bg-black/20 p-1 rounded-xl border border-white/5">
                                    {['full', 'partial'].map(t => (
                                        <button
                                            ...
                                            className={`px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest transition-all ${selectedPaymentType === t ? 'bg-amber-500 text-white shadow-lg' : 'text-amber-300 hover:text-white hover:bg-white/5'}`}
                                        >
                                            {t === 'full' ? '100%' : '50%'}
                                        </button>
                                    ))}
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-12">
                    <form onSubmit={handleSubmit} className="space-y-6 md:space-y-10">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-10">
                            {[
                                ...
                                <div key={field.key} className="space-y-2.5">
                                    <label className="text-[10px] md:text-xs font-bold text-slate-950/40 uppercase tracking-widest ml-1">{field.label}</label>
                                    <input
                                        required={field.required}
                                        type={field.type}
                                        value={formData[field.key]}
                                        onChange={(e) => setFormData({ ...formData, [field.key]: e.target.value })}
                                        className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-200 rounded-2xl focus:outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all font-bold text-slate-950 placeholder:text-slate-300"
                        ...
                        <div className="space-y-2.5">
                            <label className="text-[10px] md:text-xs font-bold text-slate-950/40 uppercase tracking-widest ml-1">Billing Address</label>
                            <textarea
                                required
                                rows="2"
                                value={formData.billingAddress}
                                onChange={(e) => setFormData({ ...formData, billingAddress: e.target.value })}
                                className="w-full px-6 py-4 bg-slate-50 border-2 border-slate-200 rounded-2xl focus:outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all resize-none font-bold text-slate-950 placeholder:text-slate-300"
                        ...
                        <div className="pt-2 md:pt-6">
                            <button
                                disabled={loading}
                                type="submit"
                                className="w-full py-6 md:py-7 bg-slate-950 text-white rounded-[2rem] md:rounded-[2.5rem] font-black text-xl md:text-2xl hover:bg-amber-800 transition-all shadow-2xl shadow-slate-950/20 flex items-center justify-center gap-4 group disabled:opacity-70"
                            >
                                {loading ? (
                        ...
                        <div className="flex flex-wrap items-center justify-center gap-x-10 gap-y-4 pt-4 text-slate-950/40">
                            <div className="flex items-center gap-2 text-[10px] md:text-xs font-black uppercase tracking-widest">
                                <CheckCircle size={14} className="text-amber-500" /> Instant Confirmation
                            </div>
                            <div className="flex items-center gap-2 text-[10px] md:text-xs font-black uppercase tracking-widest">
                                <CheckCircle size={14} className="text-amber-500" /> 24/7 Support
                            </div>
                            <div className="flex items-center gap-2 text-[10px] md:text-xs font-black uppercase tracking-widest">
                                <CheckCircle size={14} className="text-amber-500" /> Secure SSL
                            </div>
                        </div>
                    </form>
                </div>
