...
export default function MySupportTickets() {
    ...
    if (loading) return <div className="p-8 text-center text-slate-950"><Loader2 className="animate-spin mx-auto mb-2" /> Loading support...</div>;
            ...
            <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                <div>
                    <h2 className="text-xl font-bold text-slate-950 flex items-center gap-2">
                        <MessageSquare className="text-amber-600" /> Support Center
                    </h2>
                ...
                {view === 'list' && (
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-slate-950 text-white rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-amber-800 transition-colors"
                    >
                    ...
                    <div className="p-6 space-y-3">
                        {tickets.length === 0 ? (
                            <div className="text-center py-10 text-slate-400">
                                ...
                                <button onClick={() => setView('create')} className="text-amber-600 font-bold mt-2">Start a conversation</button>
                            </div>
                        ) : (
                            tickets.map(ticket => (
                                <div
                                ...
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        <h3 className="font-bold text-slate-950 group-hover:text-amber-600 transition-colors">{ticket.subject}</h3>
                        ...
                        <form onSubmit={handleCreate} className="space-y-4">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full p-4 rounded-xl border border-slate-200 outline-none focus:border-amber-500 transition-all font-bold text-slate-950"
                            ...
                            <button
                                type="submit"
                                disabled={creating}
                                className="w-full py-4 bg-slate-950 text-white rounded-xl font-bold uppercase tracking-widest hover:bg-amber-800 transition-colors disabled:opacity-50"
                            >
                            ...
                            {activeTicket.messages.map((msg, i) => (
                                <div key={i} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                                    <div className={`max-w-[80%] p-4 rounded-2xl ${msg.sender === 'user'
                                        ...
                                        : 'bg-white border border-slate-200 text-slate-950 rounded-tl-none shadow-sm'
                                        }`}>
                        ...
                        <div className="p-4 bg-white border-t border-slate-100">
                            <form onSubmit={handleSendReply} className="flex gap-2">
                                ...
                                <button
                                    ...
                                    className="p-3 bg-slate-950 text-white rounded-xl hover:bg-amber-800 disabled:opacity-50 transition-colors"
                                >
