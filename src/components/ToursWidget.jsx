    ...
    return (
        <div className="space-y-10 animate-fade-in py-4">
            <div className="flex flex-wrap gap-2 justify-center md:justify-start px-2">
                {TOUR_CATEGORIES.map(cat => (
                    <button
                        ...
                        className={`px-4 md:px-8 py-2.5 md:py-3 rounded-none text-[10px] md:text-xs font-black uppercase tracking-[0.1em] md:tracking-[0.2em] transition-all border-[4px]
                        ${activeCategory === cat
                                ? 'bg-[#FDD12C] text-black border-black'
                                : 'bg-white border-black text-black/60 hover:text-black hover:bg-[#FDD12C]/10'}`}
                    >
                        {cat}
                    </button>
                ))}
            ...
            {loading && activeCategory !== 'Custom Trip' ? (
                ...
                <div className="animate-fade-in bg-white rounded-none border-4 border-black overflow-hidden">
                    <div className="relative h-64 md:h-80 bg-navy">
                        ...
                        <button onClick={() => setSelectedTour(null)} className="absolute top-6 left-6 px-4 py-2 bg-white border-4 border-black rounded-none text-black font-black hover:bg-[#FDD12C] transition-colors flex items-center gap-2 text-sm z-10">
                            ← BACK TO TOURS
                        </button>
                        <div className="absolute bottom-6 left-8 right-8">
                            <h2 className="text-3xl md:text-5xl font-black text-white mb-2">{selectedTour.title}</h2>
                            <div className="flex flex-wrap items-center gap-4 text-white font-black uppercase tracking-widest text-[10px]">
                                <span className="flex items-center gap-1.5 bg-black px-3 py-1.5 rounded-none border-2 border-[#FDD12C]"><Clock size={16} className="text-[#FDD12C]" /> {selectedTour.duration?.days || 1} Days / {selectedTour.duration?.nights || 0} Nights</span>
                                {selectedTour.destinations && selectedTour.destinations.length > 0 && (
                                    <span className="flex items-center gap-1.5 bg-black px-3 py-1.5 rounded-none border-2 border-[#FDD12C]"><Signpost size={16} className="text-[#FDD12C]" /> {selectedTour.destinations.join(', ')}</span>
                                )}
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-8 md:p-10 space-y-10">
                        <div className="grid md:grid-cols-3 gap-8">
                            <div className="md:col-span-2 space-y-8">
                                <div>
                                    <h3 className="text-xl font-black text-black mb-3 border-b-8 border-black pb-2 uppercase tracking-tighter italic">Tour Overview</h3>
                                    <p className="text-black font-black leading-relaxed">{selectedTour.description}</p>
                                </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="space-y-4">
                                            {selectedTour.itinerary.map((day, ix) => (
                                                <div key={ix} className="bg-white border-4 border-black rounded-none p-5">
                                                    <div className="flex gap-4">
                                                        <div className="shrink-0 w-12 h-12 bg-[#FDD12C] rounded-none flex flex-col items-center justify-center text-navy font-black border-4 border-black">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-black text-sm font-bold mb-3">{day.description}</p>
                                                            {day.activities && day.activities.length > 0 && (
                                                                <div className="flex flex-wrap gap-2">
                                                                    {day.activities.map((act, i) => (
                                                                        <span key={i} className="text-[10px] font-black bg-black text-[#FDD12C] px-3 py-1 rounded-none border-4 border-black">{act}</span>
                                                                    ))}
                                                                </div>
                                                            )}
                                                        </div>
                            ...
                            <div className="space-y-6">
                                <div className="bg-black rounded-none border-[6px] border-black p-6 text-white">
                                        ...
                                        <div className="text-4xl font-black mb-6">
                                            ...
                                            <span className="text-sm font-black text-[#FDD12C] block mt-1 uppercase tracking-widest">per person</span>
                                        </div>
                                        <a href={`https://wa.me/94722885885?text=I'm interested in booking the ${selectedTour.title} package.`} target="_blank" rel="noreferrer" className="w-full bg-[#FDD12C] hover:bg-white text-black font-black py-4 rounded-none border-4 border-black transition-all flex justify-center items-center gap-2">
                                        ...
                                        <div className="bg-white border-4 border-black rounded-none p-6">
                                            {validInc.length > 0 && (
                                                <div className="mb-6">
                                                    ...
                                                    <ul className="space-y-2">
                                                        {validInc.map((inc, i) => (
                                                            <li key={i} className="text-sm text-black font-bold flex items-start gap-2"><span className="text-navy mt-0.5">✓</span> {inc}</li>
                                                        ))}
                                                    </ul>
                                                </div>
                                            )}
                                            {validExc.length > 0 && (
                                                <div>
                                                    ...
                                                    <ul className="space-y-2">
                                                        {validExc.map((exc, i) => (
                                                            <li key={i} className="text-sm text-black font-bold flex items-start gap-2"><span className="text-red-400 mt-0.5">✕</span> {exc}</li>
                                                        ))}
                                                    </ul>
                                                </div>
                                            )}
                ...
                <div className="grid grid-cols-1 animate-slide-up px-2">
                    <Link href="/custom-trip" className="group relative rounded-none overflow-hidden min-h-[380px] md:h-[450px] flex items-center justify-center bg-black border-8 border-black transition-all duration-500">
                        <div className="absolute inset-0 bg-[url('https://images.unsplash.com/photo-1546708973-4903328e19ba?q=80&w=1600')] bg-cover bg-center opacity-40 group-hover:opacity-60 transition-opacity"></div>
                        <div className="absolute inset-0 bg-black/40"></div>
                        <div className="relative z-10 text-center space-y-4 md:space-y-6 px-6 max-w-2xl py-8">
                            <div className="w-16 h-16 md:w-20 md:h-20 bg-[#FDD12C] rounded-none border-4 border-black flex items-center justify-center mx-auto mb-2 transition-transform duration-500">
                                <Signpost size={32} className="text-black md:size-10" />
                            </div>
                            ...
                            <div className="pt-2">
                                <span className="inline-flex items-center gap-3 bg-[#FDD12C] text-navy px-6 md:px-8 py-3 md:py-4 rounded-none border-4 border-black font-black text-base md:text-lg hover:bg-yellow-400 transition-colors">
                    ...
                    {filteredTours.length > 0 ? filteredTours.map((tour, idx) => {
                        ...
                        return (
                            <div
                                ...
                                className="group relative rounded-none overflow-hidden bg-white border-[6px] border-black transition-all duration-500 animate-slide-up h-[520px] flex flex-col cursor-pointer"
                            ...
                            >
                                <div className="relative h-60 overflow-hidden bg-navy flex items-center justify-center">
                                    ...
                                    <div className="absolute top-4 left-4">
                                        <div className="bg-navy border-2 border-black px-4 py-1.5 rounded-none flex items-center gap-2 text-[10px] font-black text-white uppercase tracking-widest">
                                        ...
                                        {tour.highlights && tour.highlights.length > 0 && (
                                            <div className="flex flex-wrap gap-1">
                                                {tour.highlights.slice(0, 2).map((h, i) => (
                                                    <span key={i} className="text-[9px] font-black bg-black text-[#FDD12C] px-2 py-0.5 rounded-none border border-black uppercase tracking-widest">{h}</span>
                                                ))}
                                            </div>
                                        )}
                                    </div>
                                ...
                                <div className="p-8 flex flex-col flex-1">
                                    ...
                                    <p className="text-black text-sm font-bold leading-relaxed line-clamp-2 mb-6 flex-1">{tour.description}</p>
                                    ...
                                    <div className="flex items-center justify-between pt-6 border-t-4 border-black">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-xl font-black text-navy">{converted.symbol} {converted.value.toLocaleString(undefined, { minimumFractionDigits: converted.code === 'LKR' ? 0 : 2, maximumFractionDigits: converted.code === 'LKR' ? 0 : 2 })}</span>
                                        </div>
                                        <button className="w-12 h-12 rounded-none bg-[#FDD12C] border-2 border-black text-navy flex items-center justify-center group-hover:bg-black group-hover:text-[#FDD12C] transition-all duration-300 pointer-events-none">
